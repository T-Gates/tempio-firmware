...
// ──────────── 센서 데이터 링 버퍼 ────────────
// onDataNotify(NimBLE 콜백 태스크)와 ble_get_pending_report(Arduino loop 태스크)가 동시 접근하므로 크리티컬 섹션 필요
...
// 링 버퍼에서 센서 리포트를 하나 꺼낸다. 새 데이터가 없으면 false.
